...
def main():
    ...
    for path in tqdm(cfg_fns, desc=f"collecting {len(cfg_fns)} experiments"):
        ...
        df["librarian"] = leaf_cfg.experiment.librarian.name
        df["vectorizer"] = leaf_cfg.experiment.vectorizer.name
        df["center"] = leaf_cfg.experiment.atlas.center

        # Add all fields of study considered
        fos = leaf_cfg.experiment.cartography.required_pub_conditions.fields_of_study
        for i, field in enumerate(fos):
            colname = f"fields_of_study_{i}"
            df[colname] = field
