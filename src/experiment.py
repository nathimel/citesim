...
class Experiment:
    def __init__(
        ...
        get_fullpath = lambda relpath: os.path.join(
            hydra.utils.get_original_cwd(), relpath
        )
        ...
        self.tracer = AtlasTracer(
            ...
            librarian_kwargs={
                k: v
                for k, v in config.experiment.librarian.kwargs.items()
                if v is not None
            },
            vectorizer_kwargs={
                k: v
                for k, v in config.experiment.vectorizer.kwargs.items()
                if v is not None
            },
        )
